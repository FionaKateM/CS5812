...
model_summary <- summary(CRvGE_model)
...
First the content rating and the target variable is compared, to determine whether a relationship/correlation exists between them. If there is no correlation/relationship between them, then it is to be excluded (in the cleaned data set), and therefore it is meaningless to compare the aspect ratio with other independent variables. However, I have done it just in case...
...
```{r}
# Assume you have a dataset `data` with a continuous dependent variable `DV` 
# and a categorical independent variable `Factor`
SvAR_model <- summary(lm(Movies$IMDB_Score ~ Movies$Aspect_Ratio, data = Movies))
# R-squared value from the summary
r_squared <- SvAR_model$r.squared
print(r_squared)
