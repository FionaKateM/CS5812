...
for (hidden_layers in hidden_layers_list) {
    for (lr in learning_rate_list) {
        ...
        # Save the results
        tuning_results <- rbind(tuning_results, data.frame(HiddenLayers = toString(hidden_layers), LearningRate = lr, RMSE = rmse_value, C = cor(test_mm$Gross_Earnings, rescaled_predictions)))
    }
}
itr <- itr + 1
