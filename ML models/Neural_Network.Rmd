...
testing_movies<-subset(movies_test, select = -Title)

```
#### 3. Data Normalization
```{r}
movies_test_prep <- preProcess(as.data.frame(movies_test), method=c("range"))
testing_movies_minmax <- predict(movies_test_prep, as.data.frame(movies_test))

movies_train_prep <- preProcess(as.data.frame(movies_train), method=c("range"))
training_movies_minmax <- predict(movies_train_prep, as.data.frame(movies_train))
```
...
#### 6. Decision Tree performance evaluation
...
1. One Hidden node- 65%
2. One hidden layer with 4 nodes- 56%
3. Two hidden layers with 4 nodes each- 62%

Hence, we can conclude the neural network with one hidden layer was the best among the three for predicting gross earning with accuracy 65%. 
