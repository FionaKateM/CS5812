...
# Ensure budget and Gross Earnings are numeric
movies_df <- movies_df %>%
  mutate(
...
movies_formula <- reformulate(names(movies_train[,-5]), response = 'Gross_Earnings')
```
...
svm_movies_pred <- predict(svm_movies_data, movies_test[,-5], type= "response")
