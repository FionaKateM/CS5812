...
#### 1. Normalise data
```{r}
# 1.1 normalise using min-max scaling
...
### 3.5 Plot actual values vs predicted values
plot(movies_results$actual, movies_results$svm, 
     xlab = "Actual Values", 
...
# Calculate RMSE
RMSE_value <- rmse(movies_results$svm, movies_results$actual)#rmse(DT_pred, actual_DT_values)

# calculate r-squared
rsq <- function(x, y) summary(lm(y~x))$r.squared
r2 <- rsq(movies_results$svm, movies_results$actual)#rsq(DT_pred, actual_DT_values)
