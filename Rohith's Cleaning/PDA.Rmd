...
```{r}
movies_df_imputed <- read.csv("Movies_cleaned.csv")
...
remaining_movies <- movies_final[-training_idx,]

validation_idx <- sample(n_rows * 0.2, n_rows * 0.2 * 0.5)
# filter the data frame with the training indices (and the complement)
validation_movies <- remaining_movies[validation_idx,]
testing_movies <- remaining_movies[-validation_idx,]

# write_csv(training_movies,file = "training_movies.csv")
# write_csv(validation_movies,file = "validation_movies.csv")
